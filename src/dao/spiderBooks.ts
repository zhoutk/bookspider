...
export default class SpiderBooks extends BaseDao {
            ...
            const remoteUrl = `http://search.dangdang.com/?key=${isbn}&act=input&category_path=01.00.00.00.00.00&type=01.00.00.00.00.00`
                ...
                if (result && result.length > 1) {
                    ...
                    if (rscbs && rscbs.length > 1) {
                        Object.assign(details_json, {isbn, book_name: bookName, author_name: rscbs[1], cbs: rscbs[2], cb_time: rscbs[3]})
                    }

                    return G.jsResponse(G.STCODES.SUCCESS, 'spider success.', [details_json])
                }
